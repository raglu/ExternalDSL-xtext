...
class BoilerPlateGenerator {
			...
			'''
				...
				public class Game {
				    ...
				    public Game(String gameWorldName) {
				        ...
				        deadNPCs = new ArrayList<HostileNPC> ();
				    ...
				    public void play() {
				        ...
				        while (playing) {
				            for (Player player : players) {
				                ...
				                Command command = CommandParser.getCommand(player.getName());
				    ...
				    private void goPath(Player player, String pathName) {
						...
						HostileNPC targetHostileNPC = player.getTargetHostileNPC();
						if(targetHostileNPC != null){
							targetHostileNPC.setInCombat(false);
							targetHostileNPC.setTargetPlayer(null);
						}
					...
					private void removeDeadNPCs() {
						for (HostileNPC hostileNPC : deadNPCs) {
						          npcs.remove(hostileNPC);
						          hostileNPC.getCurrentRoom().removeNPC(hostileNPC);
						          if (hostileNPC.isInCombat()) {
							          hostileNPC.getTargetPlayer().setInCombat(false);
							          hostileNPC.getTargetPlayer().setTargetHostileNPC(null);;
								      hostileNPC.setInCombat(false);
								      hostileNPC.setTargetPlayer(null);
							      }
	...
	def static generateCommandParser(IFileSystemAccess2 fsa) {
		fsa.generateFile(
			"gameDSL/CommandParser.java",
			'''
				package gameDSL;
